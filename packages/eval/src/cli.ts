...
import { EvaluationHarness, CATFACTS_TASKS, SLACK_TASKS } from "./index.js";
...
program
  .command("slack")
  .description("Run Slack Lite evaluation (S1-S4 tasks)")
  .argument("<sdk-file>", "Path to Slack Lite AgentSDK JSON file")
  .option("--openai-key <key>", "OpenAI API key (or set OPENAI_API_KEY)")
  .option(
    "--output <dir>",
  ...
  .option("--base-url <url>", "Override SDK base URL")
  .option("--runs <n>", "Number of runs per task/treatment", "2")
  .action(async (sdkFile: string, options: any) => {
    try {
      console.log(chalk.blue("🧪 Starting Slack Lite evaluation..."));
      ...
      // Create baseline documentation for Slack API
      const baselineDocs = `
...
Parameters:
- name: Channel name (lowercase, no spaces, max 21 chars)
- is_private: Create private channel (optional)
...
`;
      ...
      for (let runIndex = 0; runIndex < runs; runIndex++) {
        ...
        const harness = new EvaluationHarness(SLACK_TASKS);
      ...
      // Aggregate results (same logic as CatFacts)
      console.log(chalk.blue("\n📊 Aggregating results..."));
      ...
      // Generate markdown report
      const markdownReport = `# Slack Lite Evaluation Report
      
Generated: ${new Date().toISOString()}
Runs: ${runs} per treatment
Tasks: ${SLACK_TASKS.length} (S1-S4)
...
### S2: Get channel information
${csvData
  .filter((r) => r.taskId === "S2")
  .map(
    (r) =>
      `- ${r.treatmentName}: ${r.success ? "✅" : "❌"} (${r.duration}ms, ${r.totalTokens} tokens)`
  )
...
### S3: Create project channel
${csvData
  .filter((r) => r.taskId === "S3")
  .map(
    (r) =>
      `- ${r.treatmentName}: ${r.success ? "✅" : "❌"} (${r.duration}ms, ${r.totalTokens} tokens)`
  )
...
### S4: User information lookup
${csvData
  .filter((r) => r.taskId === "S4")
  .map(
    (r) =>
      `- ${r.treatmentName}: ${r.success ? "✅" : "❌"} (${r.duration}ms, ${r.totalTokens} tokens)`
  )
...
${aggregateStats["agent-sdk"].successRate >= aggregateStats["baseline-docs"].successRate && aggregateStats["agent-sdk"].avgTokensTotal <= aggregateStats["baseline-docs"].avgTokensTotal && aggregateStats["agent-sdk"].totalInvalidCalls <= aggregateStats["baseline-docs"].totalInvalidCalls ? "🎉 **AgentSDK successfully demonstrated improvements over baseline approach with complex API!**" : "⚠️ **Mixed results - AgentSDK shows promise but needs refinement for complex APIs.**"}

This evaluation demonstrates AgentSDK's capabilities with a realistic, multi-operation API that includes authentication, error handling, and complex workflows.
`;
      ...
      writeFileSync(
        `${outputDir}/full-results.json`,
        JSON.stringify({ aggregateStats, allRuns, csvData }, null, 2)
      );

      // Show summary
      console.log(chalk.blue("\n📈 Slack Evaluation Summary:"));
      console.log(chalk.gray("─".repeat(60)));
      ...
      console.log(
        agentSDKBetter
          ? chalk.green("🎉 AgentSDK outperformed baseline on complex API!")
          : chalk.yellow(
              "⚠️  Mixed results - complex API challenges identified"
            )
      );
    } catch (error) {
      console.error(chalk.red("❌ Slack evaluation failed:"));
      console.error(
        chalk.red(error instanceof Error ? error.message : String(error))
      );
      process.exit(1);
